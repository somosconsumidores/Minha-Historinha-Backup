...
export const useStories = () => {
  ...
  const getCharacterStory = async (characterId: string): Promise<Story | null> => {
    ...
    try {
      ...
      const chapters = [
        data.chapter_1, data.chapter_2, data.chapter_3,
        data.chapter_4, data.chapter_5, data.chapter_6,
        data.chapter_7, data.chapter_8, data.chapter_9,
        data.chapter_10
      ].filter(Boolean);
  ...
  const getUserStories = async (): Promise<Story[]> => {
    ...
    try {
      ...
      return data.map((s) => ({
        ...
        user_id: s.user_id,
        chapters: [
          s.chapter_1, s.chapter_2, s.chapter_3,
          s.chapter_4, s.chapter_5, s.chapter_6,
          s.chapter_7, s.chapter_8, s.chapter_9,
          s.chapter_10
        ].filter(Boolean),
  ...
  /** Gera capítulos + ilustrações chamando o endpoint HTTP da Edge Function */
  const generateStory = useMutation<GenerateChaptersResult, Error, GenerateChaptersInput>(
    async ({ characterId, storyTitle }) => {
      ...
      try {
        ...
        // 3) Retornar JSON com capítulos e ilustrações
